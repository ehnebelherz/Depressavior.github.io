...
from depression_detection_talks import DepressionDetection
from TalkModel import process_message
import os
...
def predictSentiment():
    ...
    return render_template("talkresult.html",result=result)
...
def predict():
    ...
    if prediction[0] == 0:
            result = 'Ваш результат теста на депрессию : Отсутствие депрессии'
    if prediction[0] == 1:
            result = 'Ваш результат теста на депрессию : Мягкая депрессия'
    if prediction[0] == 2:
            result = 'Ваш результат теста на депрессию : Умеренная депрессия'
    if prediction[0] == 3:
            result = 'Ваш результат теста на депрессию : Депрессия средней тяжести'
    if prediction[0] == 4:
            result = 'Ваш результат теста на депрессию : Глубокая депрессия'
